...
public class DefaultWebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http
                ...
                .antMatchers("/rest/server/**").authenticated().and()
                .httpBasic().and()
    ...
    public void configureGlobal(AuthenticationManagerBuilder auth) throws Exception {
        ...
        auth.inMemoryAuthentication().withUser("user").password("Pa$$w0rd").roles("kie-server", "rest-all", "admin");
        auth.inMemoryAuthentication().withUser("wbadmin").password("wbadmin").roles("admin");
        auth.inMemoryAuthentication().withUser("kieserver").password("kieserver1!").roles("kie-server");
        auth.inMemoryAuthentication().withUser("rhpamAdmin").password("Pa$$w0rd").roles("kie-server", "rest-all", "admin");
    }
